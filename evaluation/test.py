...
class Test:
    ...
    def save_images(
        ...
        tf_tn_img = metrics.tf_fn_draw(
            input[:, 2, :, :], output_tensor.squeeze(1), target_tensor.squeeze(1)
        )
        tf_tn_img.save(self.results_dir + "/visualization/" + str(count) + ".png")

    def test(self) -> None:
        test_loader = torch.utils.data.DataLoader(
            self.testset, self.batch_size, shuffle=False
        )
        tpr, fpr = [], []
        ...
        with torch.no_grad():
            for i, (data, target) in enumerate(test_loader):
                ...
                self.save_images(
                    data.cpu().data,
                    torch.round(output.cpu().data),
                    target.cpu().data.int(),
                    i,
                )
                if (
                    np.max(target.cpu().data.int().numpy()) != 0
                ):  # or np.max(torch.round(output.cpu().data).int().numpy()) != 0:
                ...
                e, outX, outY, targetX, targetY = metrics.centers_of_canals(
                    torch.round(output.cpu().data), target.cpu().int()
                )
                metrics.draw_center_of_canal(
                    data.cpu().data[:, 2, :, :], outX, outY, targetX, targetY
                ).save(self.results_dir + "/visualization_roots/" + str(i) + ".png")

                self.results["BCE Loss"].append(loss.item())
                self.results["Dice loss"].append(dice_loss_value.item())
                self.results["Jaccard index"] += (
                    [jaccard_value.item()]
                    if math.isnan(jaccard_value.item()) == False
                    else [1]
                )
                ...
                if e != "nan":
                    ...
                    self.results_roots["Jaccard index"] += (
                        [jaccard_value.item()]
                        if math.isnan(jaccard_value.item()) == False
                        else [1]
                    )
