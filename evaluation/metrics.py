...
def draw_center_of_canal(
    input: Tensor, outputX: list, outputY: list, targetX: list, targetY: list
) -> Image:
    ...
    for i in range(len(targetX)):
        if (
            img[targetY[i], targetX[i], 0] == 255
            and img[targetY[i], targetX[i], 1] != 255
        ):
...
def find_min_dist(
    output_x: list, target_x: list, output_y: list, target_y: list
) -> float:
    distances = []
    ...
    for i in range(len(shorter_x)):
        ...
        for j in range(len(longer_x)):
            dist = math.sqrt(
                (shorter_x[i] - longer_x[j]) ** 2 + (shorter_y[i] - longer_y[j]) ** 2
            )
